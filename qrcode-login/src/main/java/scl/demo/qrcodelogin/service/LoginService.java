...
import scl.demo.qrcodelogin.common.LoginConstant;
import scl.demo.qrcodelogin.entity.LoginTicket;
import scl.demo.qrcodelogin.entity.QrCodeStatusEnum;
...
public class LoginService {
    ...
    public String createQrImg() {
        ...
        cacheStoreService.put(ticketKey, loginTicket, LoginConstant.WAIT_EXPIRED_SECONDS, TimeUnit.SECONDS);
    ...
    public JSONObject scanQrCodeImg(String uuid) {
        ...
        try {
            String ticketKey = CommonUtil.buildTicketKey(uuid);
            LoginTicket loginTicket = (LoginTicket) cacheStoreService.get(ticketKey);

            // redis 中 key 过期后也可能不会立即删除
            Long expired = cacheStoreService.getExpireForSeconds(ticketKey);
            ...
            if (valid) {
                ...
                cacheStoreService.put(ticketKey, loginTicket, expired, TimeUnit.SECONDS);
...
//                cacheStore.put(CommonUtil.buildOnceTokenKey(onceToken), allowURI, LoginConstant.ONCE_TOKEN_EXPIRE_TIME, TimeUnit.SECONDS);
                cacheStoreService.put(CommonUtil.buildOnceTokenKey(onceToken), uuid, LoginConstant.ONCE_TOKEN_EXPIRE_TIME, TimeUnit.SECONDS);
    ...
    public JSONObject getQrCodeStatus(String uuid, int currentStatus) throws InterruptedException {
        ...
        try {
            ...
            if (statusEnum == QrCodeStatusEnum.CONFIRMED) {
                ...
                cacheStoreService.put(CommonUtil.buildAccessTokenKey(accessToken), loginTicket.getUserId(), LoginConstant.ACCESS_TOKEN_EXPIRE_TIME, TimeUnit.SECONDS);
