...
/**
 * @author sichaolong
 * @createdate 2023/12/22 13:40
 */
public class MinHashDemo2 {
    private Set<String> set1; // 第一个集合
    private Set<String> set2; // 第二个集合

    public MinHashDemo2(Set<String> set1, Set<String> set2) {
        this.set1 = set1;
        this.set2 = set2;
    }
    ...
    public double computeJaccardSimilarity() {
        ...
        // 初始化两个签名矩阵，用于存储每个集合中元素的哈希值
        int[] signatureMatrixSet1 = new int[numHashes];
        int[] signatureMatrixSet2 = new int[numHashes];

        for (int i = 0; i < numHashes; i++) {
            // 执行minHash算法，生成每个元素的哈希值
            int minHashValue = Integer.MAX_VALUE; // 最小哈希值初始化为整型最大值

            // 对第一个集合中的所有元素进行哈希
            for (String element : set1) {
                int hashValue = getHash(element, i);
            ...
            // 将最小哈希值保存在签名矩阵中
            signatureMatrixSet1[i] = minHashValue;

            // 对第二个集合中的所有元素进行哈希
            minHashValue = Integer.MAX_VALUE; // 重置最小哈希值为整型最大值
            ...
            // 将最小哈希值保存在签名矩阵中
            signatureMatrixSet2[i] = minHashValue;
        }

        // 计算Jaccard相似度
        double similarity = calculateSimilarity(signatureMatrixSet1, signatureMatrixSet2);

        return similarity;
    }

    private int getHash(String element, int seed) {
        // 使用一种合适的哈希函数来计算元素的哈希值，这里使用简单的字符串哈希函数
        return Math.abs((element + seed).hashCode());
    }

    private double calculateSimilarity(int[] signatureMatrixSet1, int[] signatureMatrixSet2) {
        // 统计匹配的最小哈希值数量
        int numMatches = 0;

        for (int i = 0; i < signatureMatrixSet1.length; i++) {
            if (signatureMatrixSet1[i] == signatureMatrixSet2[i]) {
                numMatches++;
            }
        }

        // 计算Jaccard相似度
        double similarity = (double) numMatches / signatureMatrixSet1.length;
    ...
    public static void main(String[] args) {
        ...
        MinHashDemo2 minHash = new MinHashDemo2(setA,setB);
        double similarity = minHash.computeJaccardSimilarity();
        ...
        List<String> listA = a.chars().distinct().boxed().map(String::valueOf).collect(Collectors.toList());
        List<String> listB = b.chars().distinct().boxed().map(String::valueOf).collect(Collectors.toList());
        ;
        MinHashDemo2 minHash2 = new MinHashDemo2(new HashSet<>(listA),new HashSet<>(listB));
        double similarity2 = minHash2.computeJaccardSimilarity();
